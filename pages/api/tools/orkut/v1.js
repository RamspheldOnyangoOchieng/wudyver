...
class OrkutApi {
  ...
  async status({ merchant, keyorkut }) {
    try {
      ...
      if (transactions.length === 0) {
        return { status: true, creator: this.creator, message: 'Belum ada transaksi terbaru.' };
      }
  ...
  async mutasi({ merchant, keyorkut }) {
    try {
      const url = `https://gateway.okeconnect.com/api/mutasi/qris/${merchant}/${keyorkut}`;
      const response = await axios.get(url);
      ...
      const transactions = response.data.data;
      if (transactions.length === 0) {
        return { status: true, creator: this.creator, message: 'Tidak ada transaksi ditemukan.' };
      }
  ...
  async saldoqr({ merchant, keyorkut }) {
    try {
      const url = `https://gateway.okeconnect.com/api/mutasi/qris/${merchant}/${keyorkut}`;
      const response = await axios.get(url);
      ...
      const transactions = response.data.data;
      if (transactions.length === 0) {
        return { status: true, creator: this.creator, message: 'Saldo tidak tersedia (tidak ada riwayat transaksi).' };
      }
    ...
    } catch (error) 'Error fetching balance:', error);
      throw new Error('Terjadi kesalahan saat mengecek saldo.');
    }
  }
}

// Daftar parameter wajib untuk setiap action.
// 'merchant' sekarang opsional untuk status, mutasi, dan saldoqr.
const availableActions = {
    deposit: ["amount", "qrcode"],
    status: [], // merchant tidak lagi wajib
    mutasi: [], // merchant tidak lagi wajib
    saldoqr: []  // merchant tidak lagi wajib
};
...
export default async function handler(req, res) {
  ...
  // Cek dan terapkan nilai default untuk action yang relevan
  ...
  if (requiredParams === undefined) { // Cek jika action tidak valid
    return res.status(400).json({
  ...
  // Validasi parameter yang wajib dari pengguna
  ...
  try {
    ...
    switch (action) {
      ...
      case 'saldoqr':
        ...
        break;
      default:
        // Cek ini sebagai fallback, meskipun sudah ditangani di atas
        return res.status(400).json({
          error: `Action tidak valid: '${action}'`,
          available_actions: Object.keys(availableActions)
        });
